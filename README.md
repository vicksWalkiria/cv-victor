...
El currículum se segmenta estratégicamente en tres versiones distintas, permitiendo optimizar el impacto y la tasa de conversión según el canal de candidatura y el tipo de cliente u oferta.

---

## 🎯 Guía Estratégica: ¿Qué versión usar en cada caso?

### 1. 📱 `cv-victor-android.pdf` (Visual Premium)
*   **Enfoque:** Especialista sénior con 15 años de experiencia en arquitectura móvil y Android nativo. Diseñado bajo la premisa de "solucionador de crisis/marrones" y consultor técnico de alto nivel.
...
### 2. 🌐 `cv-victor-fullstack-seo.pdf` (Visual Premium)
...
*   **Título:** `FullStack Developer · Technical SEO Consultant · WordPress/Laravel Specialist`
*   **Diseño:** Premium a dos columnas, con foto de perfil circular integrada.
...
### 3. 🤖 `cv-victor-ats.pdf` (ATS-Friendly)
...
*   **Cuándo usarlo:** Candidaturas automáticas en portales de empleo masivos (InfoJobs, LinkedIn Jobs, Workday, Taleo, Greenhouse, etc.). Los sistemas de filtrado automático (ATS) pueden leer y parsear este archivo con una fiabilidad del 100% sin confundir columnas.
...
├── src/
│   ├── common/
│   │   ├── data-victor.tex          # Variables comunes de contacto, teléfono y redes
...
│   ├── cv-victor-android.tex        # Origen especializado para Mobile
│   ├── cv-victor-fullstack-seo.tex  # Origen especializado para FullStack & SEO
│   └── cv-victor-ats.tex            # Origen lineal ATS
├── dist/
│   ├── cv-victor-android.pdf        # PDF generado para Android
│   ├── cv-victor-fullstack-seo.pdf  # PDF generado para FullStack/SEO
│   └── cv-victor-ats.pdf            # PDF generado para ATS
...
*   **Compilar todos los CVs a la vez:**
    ...
    make all
    ```
*   **Compilar solo una versión concreta:**
    ```bash
    make android
    make fullstack
    make ats
...
3. Compila los tres archivos `.tex` de forma limpia y comprueba que no haya errores de compilación.
4. Genera los tres PDFs finales y los guarda como un **artefacto descargable seguro** en tu interfaz de GitHub, garantizando que el repositorio siempre compile de manera impecable sin ensuciar tu historial con commits automáticos de compilación.
